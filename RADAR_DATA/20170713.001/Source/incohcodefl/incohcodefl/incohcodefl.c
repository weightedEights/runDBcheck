...
DWORD process(int inst,int npulse, void *inbuf)
{
	...
	if (hdr[10] == vars->modegroup) // modegroup test
	{
		...
		for (ilagno=1;ilagno<vars->nlags;ilagno++) 
		{
			...
			cvlags(sd,pbuf,vars->baud,vars->fraction,pind[ilagno],pmat[ilagno],code,vars->ngates);
...
static PyObject *ext_configure(PyObject *self, PyObject *args)
{
	...
	for (i=0;i<vars->nlags;i++)
		lags[i] = (float)pmat[i]*vars->sampletime;
...
PyMODINIT_FUNC
initincohcodefl(void)
{
	PyObject *m;

	m = Py_InitModule("incohcodefl", extMethods);
	if (m == NULL)
		return;
	import_array();
};
